...
// Function to compare two edges based on their weights (for sorting in descending order)
bool compareEdges(const Edge& e1, const Edge& e2) {
    return e1.weight > e2.weight;
}

// Function to find the maximum spanning weight using modified Kruskal's algorithm
int maximumSpanningWeight(vector<Edge>& edges, int numVertices) {
...
int main() {
    ...
    int maxSpanningWeight = maximumSpanningWeight(edges, numVertices);

    cout << "Maximum spanning weight (maximum budget required): " << maxSpanningWeight << "\n";
